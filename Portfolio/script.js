...
// Skills data
const skillsData = {
    labels: ['R Programming', 'Pentaho', 'Python','Html&Css','Javascript','PostgreSQL'],
    datasets: [{
        label: 'Skill Level',
        data: [80,80,85,90,75,95], // Update these values with your skill levels
        backgroundColor: [
...
const name = "ꜱᴜᴅʜᴀʀꜱᴀɴ";
let index = 0;

function typeWriter() {
    if (index < name.length) {
        nameContainer.textContent += name.charAt(index);
        index++;
        setTimeout(typeWriter, 100); // Adjust the typing speed (in milliseconds) here
    }
}
